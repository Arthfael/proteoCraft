...
if (!dir.exists(dir)) { dir.create(dir, recursive = TRUE) }
dirlist <- unique(c(dirlist, dir))
clustPrep <- sum(c(exists("clustDat"), exists("clustDatImp"))) == 2
...
if (length(filt) > 2) {
  temp <- clustDat[filt, kol]
  # For this type of PCA normalizing properly seems crucial:
  mPG <- match(row.names(temp), PG$Label)
  temp <- sweep(temp, 1, PG$"Av. log10 abundance"[mPG], "-")
  temp <- temp + rnorm(length(unlist(temp)), 0, 10^-9) # To avoid constant/zero columns, add a small random error
  ...
  if (Annotate) {
    ...
    if (tstSp %in% tst2$SpTag) { # Use pRoloc markers if our species is available in pRoloc...
    ...
    } else { # ... or make up our own
      tst <- setNames(lapply(CompGOTerms, function(x) { mPG[grep(x, PG$"GO-ID"[mPG])] }), names(CompGOTerms))
      ...
      tst <- tst[order(names(tst))]
      ClassNm <- paste(names(tst), collapse = " / ")
      for (comp in names(tst)) {
        w1 <- which(rownames(scores) %in% PG$Label[unlist(tst[[comp]])])
        scores$Classifier[w1] <- comp
      }
  ...
  compVal <- sort(compVal[which(compVal %in% unique(SubCellMark2))])
  val <- c("", compVal)
  ...
  scores$Classifier <- factor(scores$Classifier, levels = val)
  myColors <- c("grey", rainbow(length(val)-1))
  names(myColors) <- val
  colScale <- scale_colour_manual(name = "colour", values = myColors)
  scores <- rbind(scores[which(scores$Classifier == ""),],
                  scores[which(scores$Classifier %in% compVal),],
                  scores[which(scores$Classifier %in% regVal),])
  wBoring <- which(scores$Classifier == "")
  ...
  if (length(wReg)) {
    plot <- plot + geom_point(data = scores[wReg,], shape = 15) +
      geom_text_repel(data = scores[wReg,], aes(label = `Protein group`))
  }
  ...
  PL_symbolScale <- c("circle", "diamond", "square")
  names(PL_symbolScale) <- c("not regulated", "compartment marker", "regulated")  # your Classifier values
  if ("PC3" %in% colnames(scores)) {
    plot_lyPCAProt2 <- plot_ly(data = scores, x = ~PC1, y = ~PC2, z = ~PC3,
      type = "scatter3d", mode = "markers", size = ~Size,
      color = ~Classifier, colors = PL_colScale, symbol = ~Classifier, symbols = PL_symbolScale,
      text = ~paste0("<b>Protein group:</b> ", `Protein group`,
                     "\n<b>", c("Class", "Compartment")[LocAnalysis+1],
                     ":</b> ", Classifier))
  } else {
    plot_lyPCAProt2 <- plot_ly(data = scores, x = ~PC1, y = ~PC2,
                               type = "scatter", mode = "markers", size = ~Size,
                               color = ~Classifier, colors = PL_colScale, symbol = ~Classifier, symbols = PL_symbolScale,
                               text = ~paste0("<b>Protein group:</b> ", `Protein group`,
                                              "\n<b>", c("Class", "Compartment")[LocAnalysis+1],
                                              ":</b> ", Classifier))
  ...
  if (!"try-error" %in% class(tsne)) {
    ...
    rownames(scores2) <- rownames(temp)
    ...
    wBoring2 <- which(scores2$Classifier == "")
    ...
    if ("t-SNE Y3" %in% colnames(scores2)) {
      plot_lytSNE <- plot_ly(data = scores2, x = ~`t-SNE Y1`, y = ~`t-SNE Y2`, z = ~`t-SNE Y3`,
                             type = "scatter3d", mode = "markers", size = ~Size,
                             color = ~Classifier, colors = PL_colScale, symbol = ~Classifier, symbols = PL_symbolScale,
                             text = ~paste0("<b>Protein group:</b> ", `Protein group`,
                                            "\n<b>", c("Class", "Compartment")[LocAnalysis+1],
      ...
      plot_lytSNE <- plot_ly(data = scores2, x = ~`t-SNE Y1`, y = ~`t-SNE Y2`,
                             type = "scatter", mode = "markers", size = ~Size,
                             color = ~Classifier, colors = PL_colScale, symbol = ~Classifier, symbols = PL_symbolScale,
                             text = ~paste0("<b>Protein group:</b> ", `Protein group`,
                                            "\n<b>", c("Class", "Compartment")[LocAnalysis+1],
  ...
  if (!"try-error" %in% class(UMAP)) {
    ...
    rownames(UMAPlayout) <- rownames(temp)
    ...
    wBoring3 <- which(UMAPlayout$Classifier == "")
    ...
    if ("X3" %in% colnames(UMAPlayout)) {
      plot_lyUMAP <- plot_ly(data = UMAPlayout, x = ~X1, y = ~X2, z = ~X3,
                             type = "scatter3d", mode = "markers", size = ~Size,
                             color = ~Classifier, colors = PL_colScale, symbol = ~Classifier, symbols = PL_symbolScale,
                             text = ~paste0("<b>Protein group:</b> ", `Protein group`,
                                            "\n<b>", c("Class", "Compartment")[LocAnalysis+1],
      ...
      plot_lyUMAP <- plot_ly(data = UMAPlayout, x = ~X1, y = ~X2,
                             type = "scatter", mode = "markers", size = ~Size,
                             color = ~Classifier, colors = PL_colScale, symbol = ~Classifier, symbols = PL_symbolScale,
                             text = ~paste0("<b>Protein group:</b> ", `Protein group`,
                                            "\n<b>", c("Class", "Compartment")[LocAnalysis+1],
