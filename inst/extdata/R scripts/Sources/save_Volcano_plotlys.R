...
if (!dir.exists(dr)) {
  dr2 <- paste0(wd, "/", dr)
  if (dir.exists(dr2)) {
    # At this stage if it doesn't exist then it was a subdirectory
    dr <- dr2;rm(dr2)
  }
} 
# Courtesy of chatGPT:
#  - Pre-extract compact JSON specs
l <- length(myPlotLys$`Plotly plots`)
volcPlotlys <- lapply(myPlotLys$`Plotly plots`, function(x) { x$Plot })
tmpFls <- paste0(dr, "/tmp", 1:l, ".RDS")
invisible(lapply(1:l, function(i) { #i <- 1
  # keep as plain JSON string (compact, cheap to ship)
  b <- plotly::plotly_build(volcPlotlys[[i]])
...
plot_Ttls <- vapply(myPlotLys$`Plotly plots`, function(x) { x$Ttl }, "")
