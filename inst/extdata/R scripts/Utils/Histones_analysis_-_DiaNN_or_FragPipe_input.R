...
if (updateMe) {
  ...
  if ((!is.null(fl))&&(!is.na(fl))&&(file.exists(fl))&&(length(fl) == 1)&&(is.character(fl))&&(grepl("\\.tar\\.gz$", fl))) {
    try({
...
# We can load local versions for functions of interest saved in the same folder, e.g. if developing code and needing to troubleshoot a function
useLocals <- FALSE
if (useLocals) {
...
if (inputType == "FragPipe") {
  ...
  ev <- FP2MQ$Evidence
  ev$`Raw file name` <- ev$`Raw files`
  #
  rawFiles <- unique(ev$"Raw file path")
  rawFiles2 <- unique(ev$`Raw files`)
...
evSum <- evSum[, c("Modified sequence", "Modified sequence_verbose", "Modifications", "Intensity",
...
pep$Label <- parApply(parClust, pep[, labCol], 1, function(x) {
