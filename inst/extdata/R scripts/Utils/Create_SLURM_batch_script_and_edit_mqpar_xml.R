...
if ((!exists("sshsess"))||(class(sshsess) != "ssh_session")) {
  sshsess <- try(ssh_connect("bjoern22"), silent = TRUE)
  while (class(sshsess) != "ssh_session") { sshsess <- try(ssh_connect("bjoern22"), silent = TRUE) }
...
SLURM <- c("#!/bin/bash",
           ...
           "#SBATCH --mail-user=armel.nicolas@ist.ac.at",
...
wmq <- as.numeric(dlg_input(paste(c("Which version of MaxQuant should we use?",
...
out <- ssh_exec_wait(sshsess, paste0("[ -f \"", MQ_LOC, "/MaxQuantCmd.exe\" ]"))
if (out != 0) {
...
if (out != 0) {
  ...
  scp_upload(sshsess, paste0("...MaxQuant_vers_Archive/MaxQuant_", mqvers, ".zip"), paste0(LINLOC, "/", BATCH), verbose = TRUE)
  ssh_exec_wait(sshsess, paste0("unzip \"", MQloc, ".zip\" -d ", MQloc))
  ssh_exec_wait(sshsess, paste0("unlink \"", MQloc, ".zip\""))
  #
}
out <- ssh_exec_wait(sshsess, paste0("[ -f \"", MQ_LOC, "/MaxQuantCmd.exe\" ]"))
if (out != 0) {
...
if (out != 0) { stop("MaxQuant extraction failed!") } else {
  ...
  continue <- TRUE
  if (mqvers != xmlmqvers) {
    continue <- dlg_message(paste0("This mqpar.xml file was created with a different version of MaxQuant (", xmlmqvers,
  ...
  if (continue) {
    ...
    out <- capture.output(ssh_exec_wait(sshsess, paste0("ls ", BATCHDIR, " -l")))
    ...
    while (length(w) > 0) {
      scp_upload(sshsess, Raw$Full[w], BATCHDIR, verbose = TRUE)
      out <- capture.output(ssh_exec_wait(sshsess, paste0("ls ", BATCHDIR, " -l")))
      out <- grep(paste0("\\.", ext, "$"), out, value = TRUE, ignore.case = TRUE)
      out <- as.data.frame(t(sapply(strsplit(out, " "), function(x) {
        x <- unlist(x)
    ...
    while (length(w)) {
      scp_upload(sshsess, Fasta$Full[w], BATCHDIR, verbose = TRUE)
      out <- capture.output(ssh_exec_wait(sshsess, paste0("ls ", BATCHDIR, " -l")))
      out <- grep("\\.fasta$", out, value = TRUE, ignore.case = TRUE)
      out <- as.data.frame(t(sapply(strsplit(out, " "), function(x) {
        x <- unlist(x)
    ...
    if (grepl("MaxDIA", type)) {
      ...
      if (libType != 0) {
        ...
        tst <- sapply(Lib, function(x) { file.info(x)$size })
        # Create destination folder if it doesn't exist
        capture.output(ssh_exec_wait(sshsess, paste0("mkdir -p ", BATCHDIR, "/DIA_library")))
        # Upload the files
        LibNms <- gsub(".+/", "", Lib)
        w <- 1:length(Lib)
        while (length(w)) {
          out <- capture.output(ssh_exec_wait(sshsess, paste0("ls ", BATCHDIR, "/DIA_library", " -l")))
          ...
          if (length(w)) {
            ...
            scp_upload(sshsess, Lib[w], paste0(BATCHDIR, "/DIA_library"))
          }
        }
    ...
    if ((v[1] < 1)||((v[1] == 1)&&(v[2] < 6))||((v[1] == 1)&&(v[2] == 6)&&(v[3] <= 13))) {
      localcomb <- dlg_message("Do you want to use a local temporary folder?", "yesno")$res
    ...
    if (length(g)) {
      useDotNet <- c(FALSE, TRUE)[match(dlg_message("Do you want to use .NET Core?", "yesno")$res, c("no", "yes"))]
    ...
    partitions <- capture.output(ssh_exec_wait(sshsess, partcmd))
    ...
    PARTITION <- as.numeric(dlg_input(paste(c("Which of the cluster's partitions do you want to use?",
    ...
    nodes <- capture.output(ssh_exec_wait(sshsess, nodecmd))
    ...
    nodes$Total_CPUs <- as.numeric(sapply(strsplit(nodes$`CPUS(A/I/O/T)`, "/"), function(x) { rev(unlist(x))[1] }))
    ...
    bjoern <- capture.output(ssh_exec_wait(sshsess, bjoernnodecmd))
    bjoern <- gsub("^ +", "", bjoern)
    tmp <- bjoern[1]
    ...
    bjoern$Total_CPUs <- as.numeric(sapply(strsplit(bjoern$`CPUS(A/I/O/T)`, "/"), function(x) { rev(unlist(x))[1] }))
    ...
    if (length(unique(gsub("/.+", "", nodes$`CPUS(A/I/O/T)`))) > 1) {
      ...
      constraints <- as.numeric(dlg_input(msg, 1)$res)
      while (!constraints %in% 1:3) { constraints <- as.numeric(dlg_input(msg, 1)$res) }
      if (constraints == 2) {
        ...
        CONSTRAINT <- dlg_input(msg, tmp[1])$res
      ...
      if (constraints == 3) {
        NODENAME <- as.numeric(dlg_input(paste(c("Which of the nodes do you want to use?",
                                                 paste0(" - ", nodesmess, ": ", 1:length(nodesmess))), collapse = "\n"),
                                         1)$res)
    ...
    aot <- tolower(dlg_input("Should we use ahead of time (aot) compilation? (y/n)", "n")$res)
    while (!aot %in% c("y", "n")) { aot <- tolower(dlg_input("I repeat, dumbass: Should. We. Use. Ahead. Of. Time. Compilation? (y/n)", "n")$res) }
    aot <- c(TRUE, FALSE)[match(aot, c("y", "n"))]
    # Detect local SLURM scripts to identify iteration level:
    ITER <- capture.output(ssh_exec_wait(sshsess, paste0("ls ", LINLOC, "/", BATCH, "/*")))
    ...
    scp_upload(sshsess, paste0("...Archive/", BATCH, "/", c(gsub(" ", "_", xmlnm2), SLURMnm)), BATCHDIR, verbose = TRUE)
    if (dryrun) {
      ...
      scp_upload(sshsess, paste0("...Archive/", BATCH, "/", c(gsub("\\.xml", "_dryrun.xml", gsub(" ", "_", xmlnm2)), SLURMdrnm)),
                 BATCHDIR, verbose = TRUE)
    }
    Sys.sleep(1)
    setwd("...Archive/") # Just in case
    unlink(paste0("...Archive/", BATCH), recursive = TRUE, force = TRUE)
    runnow <- c(FALSE, TRUE)[match(dlg_message("Do you want to run the analysis now?", "yesno")$res, c("no", "yes"))]
  }
}
if (runnow) {
  partialrun <- FALSE
  out <- capture.output(ssh_exec_wait(sshsess, paste0("ls ", LINLOC, "/", BATCH, "/*")))
  out <- grep(topattern(paste0(LINLOC, "/", BATCH, "/")), out, value = TRUE)
  if (paste0(LINLOC, "/", BATCH, "/log_", ITER, ".out") %in% out) {
    ssh_exec_wait(sshsess, paste0("unlink \"", LINLOC, "/", BATCH, "/log_", ITER, ".out\""))
  }
  if (dryrun) {
    print("Dry run:")
    drbatchcmd <- paste0(shell, "sbatch ", LINLOC, "/", BATCH, "/", SLURMdrnm)
    ssh_exec_wait(sshsess, drbatchcmd)
    Sys.sleep(1)
    out <- capture.output(ssh_exec_wait(sshsess, paste0("ls ", LINLOC, "/", BATCH, "/*")))
    out <- grep(topattern(paste0(LINLOC, "/", BATCH, "/")), out, value = TRUE)
    ...
    sleeptm <- 1
    tasks <- capture.output(ssh_exec_wait(sshsess, paste0("cat ", LINLOC, "/", BATCH, "/log_", ITER, "_dryrun.out")))
    Sys.sleep(sleeptm)
...
if (runnow) {
  if (dryrun) {
    ...
    if (length(tasks)) {
      ...
      out <- capture.output(ssh_exec_wait(sshsess, paste0("ls ", LINLOC, "/", BATCH, "/combined/proc/")))
      ...
      if (paste0(LINLOC, "/", BATCH, "/combined/proc/#runningTimes.txt") %in% out) {
        proc <- capture.output(ssh_exec_wait(sshsess, paste0("cat ", LINLOC, "/", BATCH, "/combined/proc/#runningTimes.txt")))
        ...
        partialrun <- c(FALSE, TRUE)[match(dlg_message("Do you want to perform partial processing?", "yesno")$res,
                                           c("no", "yes"))]
  ...
  if (partialrun) {
    ...
    partialrun_step <- dlg_input(paste0("From which task (1 to ", max(proc$TaskID), ") do you want to perform partial processing?"),
    ...
    scp_upload(sshsess, paste0("...Archive/", BATCH, "/", SLURMppnm), BATCHDIR, verbose = TRUE)
    ssh_exec_wait(sshsess, paste0("sbatch ", LINLOC, "/", BATCH, "/", SLURMppnm))
  } else {
    dirs <- capture.output(ssh_exec_wait(sshsess, paste0("ls -d ", LINLOC, "/", BATCH, "/*/")))
    if (paste0(LINLOC, "/", BATCH, "/combined/") %in% dirs) { ssh_exec_wait(sshsess, paste0("rm -r \"", LINLOC, "/", BATCH, "/combined/\"")) }
    for (i in Raw$Name) {
      if (paste0(LINLOC, "/", BATCH, "/", i, "/") %in% dirs) {
        ssh_exec_wait(sshsess, paste0("rm -r \"", LINLOC, "/", BATCH, "/", i, "/\""))
        ssh_exec_wait(sshsess, paste0("unlink \"", LINLOC, "/", BATCH, "/", i, ".index\""))
      }
    }
