...
if (!file.exists(FracMapPath)) {
  ...
  # Reload local edited backup
  FracMap %<o% read.csv(paste0(wd, "/", FracMapNm,  ".csv"), check.names = FALSE)
}
...
if (exists("frMap2")) { rm(FracMap3) }
server <- function(input, output, session) {
  stop("Check that app still works!!!")
  frMap2 <- frMap <- FracMap
  ...
  tst <- sum(FracMap$Use)
  if (!tst) { shinyjs::disable("saveBtn") }
  if (tst) { shinyjs::enable("saveBtn") }
  ...
  output$FracMap_data <- DT::renderDT({ frMap },
                                      FALSE,
                                      escape = FALSE,
                                      selection = "none",
                                      editable = TRUE,
  ...
  observeEvent(input$FracMap_data_cell_edit, {
    kol <- colnames(frMap)[input$FracMap_data_cell_edit$col+1]
    frMap2[input$FracMap_data_cell_edit$row, kol] <<- input$FracMap_data_cell_edit$value
  })
  observeEvent(input$saveBtn, {
    for (k in c("Use", "PTM-enriched")) {
      root <- gsub("-", "", k)
      frMap2[[k]] <- sapply(rws, function(x) { input[[paste0(root, "_", as.character(x))]] })
    }
...
if (file.exists(SamplesMapPath)) {
  SamplesMap %<o% suppressWarnings(read.csv(SamplesMapPath, check.names = FALSE))
  SamplesMap <- SamplesMap[which(SamplesMap$MQ.Exp %in% FracMap$`Parent sample`),]
  # Fix older bug:
  if ("MQ.exp" %in% colnames(SamplesMap)) { colnames(SamplesMap)[which(colnames(SamplesMap) == "MQ.exp")] <- "MQ.Exp" }
} else {
  SamplesMap %<o% data.frame("MQ.Exp" = exp,
                             "Negative Filter" = FALSE,
                             check.names = FALSE)
  if (MakeRatios) {
    SamplesMap$"Ratios group" <- 1
    SamplesMap$"Reference" <- FALSE
  }
}
# if ("Order" %in% colnames(SamplesMap)) {
#   u <- unique(SamplesMap$Order)
#   u <- u[which(!u %in% 1:nrow(SamplesMap))]
#   if (length(u) < nrow(SamplesMap)) { SamplesMap$Order <- 1:nrow(SamplesMap) }
# } else { SamplesMap$Order <- 1:nrow(SamplesMap) }
smplMapKol <- c("Reference", "Negative Filter", "Use")
for (kol in smplMapKol) {
...
allIDs <- as.character(sapply(smplMapKol, function(x) {
  paste0(x, "___", 1:nrow(SamplesMap))
}))
...
server <- function(input, output, session) {
  stop("Check that app still works!!!")
  # Create copies table
  smplMap <- smplMap2 <- SamplesMap[, which(!colnames(SamplesMap) %in% "New name")] # This column is deprecated and ignored
  colnames(smplMap)[which(colnames(smplMap) == "MQ.Exp")] <- "Parent sample"
  IDs <- c()
  MSG <- reactiveVal("")
  for (kol in c("Reference", "Negative Filter", "Use")) {
    if (kol %in% colnames(SamplesMap)) {
      smplMap[[kol]] <- shinyCheckInput(SamplesMap[[kol]], kol)
      IDs <- c(IDs, paste0(kol, "_", seq_len(nrow(smplMap))))
      stopifnot(length(IDs) == length(unique(IDs)))
    }
  }
  output$Message <- renderUI({ em(" ") })
  # smplMap$Order <- shinySelectInput(SamplesMap$Order,
  #                                   "Order",
  #                                   1:nrow(SamplesMap),
  #                                   "150px")
  output$smplMap_data <- DT::renderDT(smplMap,
                                      FALSE,
                                      escape = FALSE,
                                      selection = "none",
                                      editable = TRUE,
 ...
 # Observe events
  if (length(IDs)) {
    sapply(IDs, function(id) {
      ...
      observeEvent(input[[id]], {
        SamplesMap[x, kol] <<- as.logical(input[[id]])
        if (kol == "Use") {
  ...
  observeEvent(input$smplMap_data_cell_edit, {
    smplMap2[input$smplMap_data_cell_edit$row,
             input$smplMap_data_cell_edit$col+1] <- input$smplMap_data_cell_edit$value
  })
  observeEvent(input$expOrder, {
    assign("expOrder", input$expOrder, envir = .GlobalEnv)
  })
  ...
  #     #cat("Event", id, "\n")
  #     #cat(input[[id]], "\n")
  #     SamplesMap[i, kol] <<- as.logical(input[[id]])
  #   })
  # })
  observeEvent(input$saveBtn, {
    for (k in smplMapKol) {
      smplMap2[[k]] <- sapply(1:nrow(SamplesMap), function(x) { input[[paste0(k, "___", x)]] })
    }
    assign("smplMap2", smplMap2, envir = .GlobalEnv)
    stopApp()
  })
  #observeEvent(input$cancel, { stopApp() })
  session$onSessionEnded(function() { stopApp() })
...
# Annotate
setwd(wd)
Annotate %<o% c(TRUE, FALSE)[match(dlg_message("Annotate the databases and protein groups tables?\n(This is required for GO terms enrichment analysis)", "yesno")$res, c("yes", "no"))]
...
if (Annotate) {
  if (FndAnnotFl) {
    ...
    if (!length(AnnotFls)) {
      ...
      Parsed_annotations %<o% lapply(AnnotFls, function(x) { #x <- AnnotFls[1]
...
server <- function(input, output, session) {
  ...
  output$GO <- renderUI({
    ...
    if (Annotate) {
      lst <- list(
        list(fluidRow(h4("GO terms enrichment")),
...
if (tst) {
  ...
  Exp %<o% "Exp1"
  ev$Experiment <- Exp
  SamplesMap <- data.frame("MQ.Exp" = Exp,
                           "Ratios group" = 1,
                           "Reference" = TRUE,
                           "Negative Filter" = FALSE,
                           "Use" = TRUE,
                           check.names = FALSE)
  write.csv(SamplesMap, "SamplesMap.csv", row.names = FALSE)
  colnames(SamplesMap)[which(colnames(SamplesMap) == "MQ.Exp")] <- "Experiment"
}
...
if (Update_Prot_matches) {
  #Reuse_Prot_matches %<o% FALSE
  # if ("evmatch.RData" %in% list.files()) {
  #   msg <- "Some backed-up protein matches are available in the folder, do you want to reuse them?\n"
  #   Reuse_Prot_matches <- c(TRUE, FALSE)[match(dlg_message(msg, "yesno")$res, c("yes", "no"))]
  # }
  if (Reuse_Prot_matches) {
    loadFun("evmatch.RData")
    temp <- evmatch
  } else {
    temp <- ProtMatch2(Seq = unique(ev$Sequence), DB = db,
                       cl = parClust) # (ignore the warning for now until we remove contaminant evidences)
  }
  w <- which(ev$Sequence %in% temp$Sequence)
  m <- match(ev$Sequence[w], temp$Sequence)
  stopifnot(sum(ev$Sequence[w] != temp$Sequence[m]) == 0)
  if (!"Proteins" %in% colnames(ev)) { ev$Proteins <- "" } else {
    ...
    tst2 <- parSapply(parClust, tst2, paste, collapse = ";")
    wN <- which(tst1 != tst2)
    ViewTst <- FALSE
    if ((length(wN))&&(ViewTst)) {
      # Below some code to help with investigations...
      tst <- data.frame(Seq = ev$Sequence[w[wN]],
                        Orig = ev$Proteins[w[wN]],
                        Corr = temp$Proteins[m[wN]])
      tst$Orig <- strsplit(tst$Orig, ";")
      #View(tst)
      tst$Corr <- strsplit(tst$Corr, ";")
      tst$In_Orig_only <- apply(tst[, c("Orig", "Corr")], 1, function(x) { x[[1]][which(!x[[1]] %in% x[[2]])] })
      tst$In_Corr_only <- apply(tst[, c("Corr", "Orig")], 1, function(x) { x[[1]][which(!x[[1]] %in% x[[2]])] })
      ...
      View(tst[w,])
      apply(tst[, c("Orig", "Corr")], 1, function(x) { x[[1]][which(!x[[1]] %in% x[[2]])] })
    }
    tst <- sum(tst1 != tst2)
    if (tst) {
      msg <- paste0("Corrected ", tst, " out of ", length(w), " assignments!
Prior investigations have identified 3 cases:
 1) Protein present in original column but not corrected results:
   ...
   b) More rarely, the accession is present in the db... but for every peptide we have checked so far the protein sequence was not compatible with it!!!
...
")
  ...
  ev$Proteins <- parApply(parClust, tmp, 1, function(x) { paste(unique(unlist(x)), collapse = ";") })
}
w <- which(ev$Proteins == "")
if (length(w)) {
...
if (runPepper) {
  ...
  txt <- paste0(sapply(Exp, function(x) { paste0(x , " = sd(", x, ")") }), collapse = ", ")
  for (i in 1:2) {
    tmp2 <- copy(tmp)
...
if (length(Exp) > 1) {
  ...
  tmp2D <- tmp2D[which(sapply(tmp2D, function(x) { x$Success }))]
...
PG_assembly %<o% PG_assemble(pep, DB = db, Ev = ev, N.clust = N.clust, Custom_PGs = custPGs, Npep = NPep,
                             cl = parClust)
...
if (Annotate) {
  ...
  for (i in annot.col) { temp[[i]] <- sapply(temp[[i]], function(x) { paste(unique(unlist(x)), collapse = ";") }) } # Do not use sort here or it will break the correspondance between "GO" and "GO-ID"
  PG[, annot.col] <- temp[match(PG$id, temp$Group.1), annot.col]
  tst1 <- unlist(strsplit(PG$`GO-ID`, ";"))
  tst2 <- unlist(strsplit(PG$GO, ";"))
  tst3 <- data.table(A1 = tst1, A2 = tst2)
  tst3 <- tst3[, list(x = unique(A2)), by = list(Group.1 = A1)]
  tst3 <- as.data.frame(tst3)
  stopifnot(length(tst3$x) == length(unique(tst3$x)), "character" %in% class(tst3$x))
  ...
  pep[, annot.col] <- PG[match(as.integer(gsub(";.*", "", pep$`Protein group ID`)), PG$id), annot.col]
}
...
if (GO_filt) {
  for (goID in GO_filter) { #goID <- GO_filter[1]
    ...
    if (sum(gofilter %in% AllTerms)) {
      ...
      #which(sapply(strsplit(PG$`GO-ID`[-wtst],";"), function(x) { sum(x %in% gofilter) }) != 0)
      #which(sapply(strsplit(PG$`GO-ID`[wtst],";"), function(x) { sum(x %in% gofilter) }) == 0)
    }
  }
}
...
test <- sapply(g, function(x) { length(is.all.good(PG[[x]])) })
print(test)
...
if (MakeRatios) {
  ...
  if (PTMriched) {
    ...
    for (ptm in EnrichedPTMs) { #ptm <- EnrichedPTMs[1]
      ...
      ptmpep$Name[w] <- sapply(strsplit(gsub("[/,;].+$", "", ptmpep[w, paste0(Ptm, "-site(s)")]), " "), function(x) {
      ...
      if (!length(w)) {
        ...
        for (id in IDs) { #id <- IDs[1]
          ...
          if (nrow(s)) {
            ...
            tempev <- tempev[which(sapply(tempev, class) == "data.frame")]
            if (length(tempev)) {
              ...
              for (exp in names(tempev)) { #exp <- names(tempev)[1]
                ...
                Coverage(P, tmp$"Modified sequence", Mode = "Align2", display = FALSE, title = ttl, save = c("jpeg", "pdf"),
                         intensities = tmp$`log10(Intensity)`, maxInt = mxInt)
              ...
              if (length(tempev) > 1) {
                ...
                for (j in 1:nrow(comb)) { #j <- 1
                  ...
                  if (sum(c(nrow(s1) > 0, nrow(s2) > 0)) == 2) {
                    ...
                    if (length(wtst)) {
                      kount <- kount + 1
                      temp3 <- s1[which(s1$"Modified sequence" %in% s2$"Modified sequence"), c("Modified sequence", "Match" , "value")]
                      colnames(temp3)[which(colnames(temp3) == "value")] <- "log10(X LFQ)"
                      temp3$"log10(Y LFQ)" <- s2$value[match(apply(temp3[, c("Modified sequence", "Match")], 1, paste, collapse = "___"),
                                                             s2$tmp)]
                ...
                if (kount) {
                  ...
                  temp2$"log10(X/Y)" <- temp2$`log10(X LFQ)` - temp2$`log10(Y LFQ)`
                  ...
                  ggsave(paste0(gsub(":|\\*|\\?|<|>|\\||/", "-", ttl), ".pdf"), plot,
                         dpi = 300, width = 10, height = 10, units = "in")
                  setwd(wd)
                  temp2$X <- paste0("X = ", gsub(" \\(X\\)$", "", temp2$X))
                  temp2$Y <- paste0("Y = ", gsub(" \\(Y\\)$", "", temp2$Y))
                  Xext <- seqL <- nchar(P)
                  Yext <- max(temp2$`log10(X/Y)`) - min(temp2$`log10(X/Y)`)+1
                  sk <- Xext/(3*Yext)
                  temp2$MW <- sapply(temp2$`C-terminal extent`, function(x) { mw(substr(P, 1, x)) })
                  tst <- data.frame(AA = unlist(strsplit(P, "")))
                  tst$MW <- sapply(tst$AA, mw)
                  ...
                  ySum <- summary(is.all.good(temp2$`log10(X/Y)`))
                  ...
                  tst <- aggregate(temp2$Match, list(temp2$X, temp2$Y), function(x) { length(unique(x)) })
                  ...
                  if (min(tst$x) >= 20) {
                    plot <- plot + geom_smooth(aes(x = (`C-terminal extent`+Match)/2, y = `log10(X/Y)`), alpha = 0.1, linewidth = 0.5)
                  }
                  plot <- plot + geom_segment(aes(x = Match, xend = `C-terminal extent`, text = `Modified sequence`,
                                                  y = `log10(X/Y)`, yend = `log10(X/Y)`, colour = `log10(X/Y)`), linewidth = 1)
                  if (nr) {
                  ...
                  plot <- plot + coord_fixed(round(sk*2)) +
                    ...
                    facet_grid(Y~X) + ylab("log10(X/Y)") +
                    xlim(xLim[1]-5, xLim[2]+5) +
                    ylim(yLim[1], yLim[2]) + xlab("Amino acid") +
                    theme_bw() + theme(strip.text.y = element_text(angle = 0, vjust = 0.5, hjust = 0)#,
                                        #strip.text.x = element_text(angle = 90, vjust = 0.5, hjust = 0)
                     ) + ggtitle(ttl)
                  ttl2 <- paste0(dir, "/", nm2, "/Ratios/", gsub(":|\\*|\\?|<|>|\\||/", "-", ttl))
                  plotLY <- ggplotly(plot)
                  tst <- try(saveWidget(partial_bundle(plotLY), paste0(ttl2, ".html")), silent = TRUE)
                  if ("try-error" %in% class(tst)) { tst <- try(saveWidget(plotLY, paste0(ttl2, ".html")), silent = TRUE) }
                  if (WorkFlow == "Band ID") {
...
if (globalGO) {
  ...
  if (class(temp) != "logical") {
    temp <- temp$GO_terms
    temp$"Protein group IDs" <- sapply(temp$`Protein table row(s)`, function(x) {
  ...
  for (grp in Grps) {
    ...
    if (OK) {
      if (length(w) > VennMx) {
        ...
        tmp <- setNames(sapply(VennExp, function(x) { paste(c(x, rep(" ", 200-nchar(x))), collapse = "") }), VennExp)
...
if (length(Exp) > 1) {
  ...
  if (!dir.exists(dir)) { dir.create(dir, recursive = TRUE) }
  if (MakeRatios) {
    I <- list(Global = unique(SamplesMap$`Ratios group`))
    if (length(unique(SamplesMap$`Ratios group`)) > 1) { for (i in unique(SamplesMap$`Ratios group`)) { I[[as.character(i)]] <- i } }
  } else {
    I <- list(Global = 1)
  }
  Heatmaps <- NHClust <- NVClust <- list()
  KlustKols %<o% c()
  ImputeKlust <- TRUE
  MaxHClust <- min(c(floor(nrow(PG)/5), 100)) # We want at most 20 clusters
  ...
  for (i in names(I)) { #i <- names(I)[1]
    if (MakeRatios) {
      j <- unlist(I[i])
      Samples <- SamplesMap$Experiment[which(SamplesMap$`Ratios group` %in% j)]
    } else {
      j <- "Global"
      Samples <- SamplesMap$Experiment
    }
    if (i == "Global") {
      #opt <- sapply(Samples, function(x) { paste(c(x, rep(" ", 200-nchar(x))), collapse = "") })
      #rs <- dlg_list(opt, opt, TRUE, "Select which samples you want to cluster in the global heatmap:")$res
      #while (length(rs) < 2) { rs <- dlg_list(opt, opt, TRUE, "Select at least 2 samples to cluster in the global heatmap:")$res }
      #Samples <- Samples[match(rs, opt)]
      MaxVClust <- length(Samples)
      VClustScl <- setNames(1:MaxVClust, paste0("Cluster", 1:MaxVClust))
      VClustScl <- setNames(rainbow(MaxVClust), VClustScl)
    }
    kol <- paste0(PG.int.col, Samples)
    Samples <- Samples[which(kol %in% colnames(PG))]
    kol <- kol[which(kol %in% colnames(PG))]
    if (length(kol) > 1) {
      GoOn <- TRUE
      msg <- paste0("Creating ", c("", paste0(i, " "))[(i == "Global")+1], "heatmap",
                    c(paste0(" for ", gsub("___", " ", i)), "")[(i == "Global")+1], ".")
      #ReportCalls <- AddMsg2Report(Space = FALSE)
      cat(paste0(msg, "\n"))
      temp <- set_rownames(set_colnames(PG[, kol], gsub("___", " ", gsub(topattern(PG.int.col), "", kol))),
                           PG$Label)
      Gr <- setNames(rep(1, length(Samples)), Samples)
      tst <- apply(temp, 1, function(x) { length(is.all.good(x)) })
      filt <- rep(FALSE, nrow(temp))
      temp <- temp[which((tst > 0)&(PG$`Potential contaminant` != "+")),]
      if (ImputeKlust) {
        temp2 <- try(Data_Impute2(temp, Gr), silent = TRUE)
        if ("try-error" %in% class(temp2)) {
          msg <- paste0("Imputation failed: ", temp2[1])
          #ReportCalls <- AddMsg2Report(Space = FALSE)
          cat(msg, "\n")
          msg <- "Filtering instead..."
          cat(msg, "\n")
          GoOn <- FALSE
        } else {
      ...
      if (GoOn) {
        ...
        temp <- temp[w,]
        if (ImputeKlust) { imputed <- imputed[w,] }
        temp <- sweep(temp, 1, rowMeans(temp), "-")
        ...
        hnm <- paste0("Protein groups-level clusters number analysis - ", gsub("___", " ", i))
        NHClust[[i]] <- MaxHClust
        Straps <- 10
        NVClust[[i]] <- length(Samples)
        # Here we want to optimize the number of clusters
        # Apply the same method for optimization for any clustering method
        # Number of clusters should not depend on method
        ...
        #poplot(hplot)
        ggsave(paste0(dir, "/", hnm, ".jpeg"), hplot, dpi = 150)
        ggsave(paste0(dir, "/", hnm, ".pdf"), hplot, dpi = 150)
        # Apply cutoffs
        if (KlustMeth == 1) {
        ...
        # Create intermediate dendrogam plots - we will modify them later
        ...
        vlabs <- vddata$labels
        vlabs$Cluster <- as.factor(VClusters[[i]][match(vlabs$label, gsub("___", " ", Samples))])
        ...
        # Rotate samples-level dendrogram:
        vlabs$y <- vlabs$x
        vlabs$x <- -0.1
        x <- vddata$segments$x
        y <- vddata$segments$y
        xend <- vddata$segments$xend
        yend <- vddata$segments$yend
        vddata$segments$y <- x
        vddata$segments$x <- y
        vddata$segments$yend <- xend
        vddata$segments$xend <- yend
        vddata$labels <- NULL
        # Order labels by order or appearance
        ...
        temp2 <- set_colnames(reshape::melt.data.frame(temp), c("Label", "Sample", "value"))
        ...
        temp2[, c("Xmin", "label2")] <- hlabs[match(temp2$Label, hlabs$label), c("x", "label2")]
        temp2$Xmin <- temp2$Xmin-1
        temp2$Xmax <- temp2$Xmin+1
        temp2$Ymax <- vlabs$y[match(temp2$Sample, vlabs$label)]
        temp2$Ymin <- temp2$Ymax-1
        kol1 <- c("id", "Common Name (short)")
        kol2 <- c("PG id", "Common Name (short)")
        temp2[, kol2] <- PG[match(temp2$`Leading protein IDs`, PG$`Leading protein IDs`), kol1]
        temp2$label2 <- apply(temp2[, kol2], 1, paste, collapse = ": ")
        temp2$Cluster <- tempClust[match(temp2$Label, names(tempClust))] # Works for both methods, yay!
        temp2$Cluster <- factor(temp2$Cluster, levels = 1:NHClust[[i]])
        Xscale <- max(temp2$Xmax)
        Yscale <- length(unique(temp2$Sample))
        ...
        #SamplesClust$Group <- as.factor(paste0("Group", as.character(Gr[match(SamplesClust$label, names(Gr))]))) # No! This would be good for a script with replicates, not without!!!
        SamplesClust$Group <- as.factor(SamplesClust$label)
        ...
        # Create plot
        nm <- paste0("Clust. heatmap - ", gsub("___", " ", i))
        ...
        heatmap.plot <- heatmap.plot +
          geom_segment(data = hddata$segments, linewidth = 0.5,
                       aes(x = x-0.5, y = y+Yscale+1, xend = xend-0.5, yend = yend+Yscale+1)) +
          geom_text(data = hlabs, aes(x = x-0.5, label = label2, color = Cluster),
                    y = Yscale+0.05, angle = 90, cex = 0.5, hjust = 0, vjust = 0)
        #poplot(heatmap.plot, 12, 20)
        if (KlustMeth == 2) {
          ...
          heatmap.plot <- heatmap.plot +
            geom_rect(data = Clutst, aes(xmin = x, xmax = xend, ymin = 0, ymax = Yscale, colour = Cluster), fill = NA) +
            geom_text(data = Clutst, aes(x = mid, y = Yscale/2, label = Cluster, colour = Cluster),
                      hjust = 0.5, vjust = 0.5, cex = 5)
        }
        #poplot(heatmap.plot, 12, 20)
        # Vertical
        heatmap.plot <- heatmap.plot +
          geom_segment(data = vddata$segments, linewidth = 0.5,
                       aes(x = -Xscale*0.07-x, y = y-0.5, xend = -Xscale*0.07-xend, yend = yend-0.5)) +
          new_scale_color() + VcolScale +
          geom_text(data = vlabs, aes(y = y-0.3, label = label, colour = Cluster),
                    x = -0.1, hjust = 1, vjust = 0.5, cex = 2.5)
        #poplot(heatmap.plot, 12, 20)
        # Title, axis labels, colour scale annotations
        xCntr <- mean(Xlim)
        yCntr <- mean(Ylim)
        xScale2 <- Xlim[2]-Xlim[1]
        yScale2 <- Ylim[2]-Ylim[1]
        GrLab <- data.frame(label = c(nm, "Sample",
                                      paste0("Chi-squared contingency test P-value: ", round(ClustChiSqTst$p.value, 5)),
                                      c("Protein group", "Protein group\n(marks = PG id + Common name)")[prot.list.marks+1], "Min", "Max"),
                            x = c(xCntr, -max(vddata$segments$x)-Xscale*0.11, -max(vddata$segments$x)-Xscale*0.095,
                                  xCntr, min(temp2b$Xmin), max(temp2b$Xmin)),
                            y = c(Ylim[1]+yScale2*0.95, Yscale*0.5, Yscale*0.5,
                                  Ylim[1]+yScale2*0.9, temp2b$Ymin[1]-1, temp2b$Ymin[1]-1),
        ...
        if (i == "Global") {
          poplot(heatmap.plot, 12, 20)
          write.csv(temp2, paste0(dir, "/", nm, ".csv"), row.names = FALSE)
        }
        ggsave(paste0(dir, "/", nm, ".jpeg"), heatmap.plot, dpi = 600, width = 20, height = 12, units = "in")
        ggsave(paste0(dir, "/", nm, ".pdf"), heatmap.plot, dpi = 600, width = 20, height = 12, units = "in")
        #ReportCalls <- AddPlot2Report(Title = nm)
        #system(paste0("open \"", dir, "/", nm, ".jpeg", "\""))
        #system(paste0("open \"", dir, "/", nm, ".pdf", "\""))
      }
    }
  }
  kol <- paste0(PG.int.col, Exp)
  kol <- kol[which(kol %in% colnames(PG))]
...
for (QuantType in QuantTypes) { #QuantType <- "Coverage" #QuantType <- QuantTypes[1]
  ...
  if (length(g)) {
    ...
    if (nrow(temp)) {
      ...
      clusterCall(parClust, function() library(plotly))
      clusterCall(parClust, function() library(AnnotationDbi))
      clusterCall(parClust, function() library(htmlwidgets))
...
if (plotPepProf) {
  ...
  #
  MainDir <- paste0(wd, "/Sorting plots")
  clusterExport(parClust, list("plotsDF", "tstorg2", "wd", "MainDir"), envir = environment())
  cat("Plotting peptide sorting plots\n")
  pepSortTst <- parLapply(parClust, 1:nrow(plotsDF), function(ii) { #ii <- 1
    ...
    intscale <- intmax-intmin
    xmax <- nr*18/15
    ttl <- paste0("Peptide ", gsub(" - $", "", root), " sorting plots - ", exp)
    if (plotMode == "All") {
      catnm <- "Category"
      filt <- 1:nr
    }
...
if (NegFilt) {
  ...
  PG$"Direct identification in negative filter sample(s)" <- sapply(strsplit(PG$`Evidence IDs`, ";"), function(x) { sum(x %in% e$id) }) > 0
...
# addWorksheet(wb, "tmp")
# tmpFl <- temp_xlsx()
#w <- which(sapply(Styles, class) == "Style")
...
temp <- data.frame(Parameter = names(AnalysisParam),
                   Value = sapply(AnalysisParam, function(x) { paste(unlist(x), collapse = ";") }),
                   row.names = NULL)
...
MatMetCalls %<o% list(Calls = list("read_docx()",
                                   paste0("body_add_fpar(MatMet, fpar(ftext(\"Sample", c("", "s")[(nrow(SamplesMap) > 1)+1],
                                          " preparation\", prop = WrdFrmt$Section_title), fp_p = WrdFrmt$just))")),
                      Texts = c())
ObjNm <- "ProcessedByUs"
#if ((ReUseAnsw)&&(ObjNm %in% AllAnsw$Parameter)) { assign(ObjNm, AllAnsw$Value[[match(ObjNm, AllAnsw$Parameter)]]) } else {
msg <- "Do you know enough details about how the samples were processed to write a Materials & Methods?"
tmp <- c(TRUE, FALSE)[match(dlg_message(msg, "yesno")$res, c("yes", "no"))]
if (is.na(tmp)) { tmp <- FALSE }
ObjNm %<c% tmp
#}
if (ProcessedByUs) {
  if (LabelType == "LFQ") { WetLabMeth <- try(MatMet_WetLab(exp.map = SamplesMap), silent = TRUE) }
...
MatMetCalls$Texts <- c(MatMetCalls$Texts, WetLabMeth)
NewL <- length(MatMetCalls$Texts)
MatMetCalls$Calls <- append(MatMetCalls$Calls, paste0("body_add_fpar(MatMet, fpar(ftext(MatMetCalls$Texts[", NewL,"], prop = WrdFrmt$",
                                                      c("Body", "Template_text")[(WetLabMeth == "TEMPLATE")+1], "_text), fp_p = WrdFrmt$just))"))
...
OldL <- length(MatMetCalls$Texts)
LCMSMeth <- try(MatMet_LCMS(), silent = TRUE) ## Slow the 1st time, but need be done only once; if rerun, the methods files will still be there, making the process much faster
if ((("try-error" %in% class(LCMSMeth)))||(is.null(LCMSMeth))&&("mzML" %in% gsub(".*\\.", "", rawFiles))) {
  LCMSMeth <- try(MatMet_LCMS(RawFiles = gsub("\\.mzML", ".raw", rawFiles)), silent = TRUE)
  if ((("try-error" %in% class(LCMSMeth)))||(is.null(LCMSMeth))&&("mzML" %in% gsub(".*\\.", "", rawFiles))) {
    LCMSMeth <- try(MatMet_LCMS(RawFiles = gsub("\\.mzML", ".d", rawFiles)), silent = TRUE)
  }
}
...
MatMetCalls$Texts <- c(MatMetCalls$Texts, LCMSMeth)
NewL <- length(MatMetCalls$Texts)
for (i in (OldL+1):NewL) {
  MatMetCalls$Calls <- append(MatMetCalls$Calls, paste0("body_add_fpar(MatMet, fpar(ftext(MatMetCalls$Texts[", i,"], prop = WrdFrmt$",
                                                        c("Body", "Template_text")[(WetLabMeth == "TEMPLATE")+1], "_text), fp_p = WrdFrmt$just))"))
}
MatMetCalls$Calls <- append(MatMetCalls$Calls, "body_add_par(MatMet, \"\", style = \"Normal\")")
...
Org <- aggregate(w, list(db[w, kol]), length)
...
if (SearchSoft == "MAXQUANT") {
  ...
  tsts <- setNames(as.logical(toupper(c(MBR, SecPep, DPep))), c("Match-between-Runs", "Second Peptides", "Dependent Peptides"))
...
if (SearchSoft == "DIANN") {
  ...
  if (length(VarMd)) {
    ...
    VarMd$Type <- "Variable"
    VarMd$"Full name" <- apply(VarMd[, c("UniMod", "Delta mass")], 1, function(x) {
  ...
  Libraries <- grep("^--lib", DIANNCall2)
  if (length(Libraries)) {
    Libraries <- DIANNCall2[Libraries+1]
    Libraries <- Libraries[which(Libraries != "")]
  }
  nL <- length(Libraries)
  LibFree <- (nL == 0)+1
  DatAnalysisTxt <- paste0(c("The r", "R")[moult], "aw file", c(" was", "s were")[moult], " searched in ", names(SearchSoft))
  if ((length(SearchSoftVers))&&(nchar(SearchSoftVers))) {
    DatAnalysisTxt <- paste0(DatAnalysisTxt, " version ", SearchSoftVers)
  }
...
NewL <- length(MatMetCalls$Texts)
MatMetCalls$Calls <- append(MatMetCalls$Calls, paste0("body_add_fpar(MatMet, fpar(ftext(MatMetCalls$Texts[", NewL,"], prop = WrdFrmt$",
                                                      c("Body", "Template_text")[(DatAnalysisTxt == "TEMPLATE")+1], "_text), fp_p = WrdFrmt$just))"))
