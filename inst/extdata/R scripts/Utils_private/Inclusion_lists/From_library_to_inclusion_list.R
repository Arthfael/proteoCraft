...
msg <- "Select a DIA-NN predicted library file (tsv format only!)"
#filt <- matrix(c("DIA-NN tsv library", "*.tsv"), ncol = 2)
#LibFl <- normalizePath(choose.files(paste0(wd, "/*.tsv"), msg, multi = FALSE, filt, 1), winslash = "/")
LibFl <- rstudioapi::selectFile(msg,
                                path = paste0(wd, "/*.tsv"),
                                filter = ".tsv library file (*.tsv)")
...
msg <-  "Select a DIA-NN report file containing peptide identifications from a sample with the desired gradient"
#filt <- matrix(c("DIA-NN report tsv file", "*.tsv"), ncol = 2)
#ObsFl <- choose.files(paste0(wd, "/*.tsv"), msg, multi = FALSE, filt, 1)
ObsFl <- rstudioapi::selectFile(msg,
                                path = paste0(wd, "/*.tsv"),
                                filter = "DiaNN .tsv report file (*.tsv)") # Currently this does not support parquet but could be added easily
...
msg <- "Select a fasta file of proteins for which you want to write the inclusion list"
#filt <- matrix(c("FASTA file", "*.fasta"), ncol = 2)
#FiltFastaFl <- normalizePath(choose.files(paste0(wd, "/*.fasta"), , multi = FALSE, filt, 1), winslash = "/")
FiltFastaFl <- rstudioapi::selectFile(msg,
                                      path = paste0(wd, "/*"),
