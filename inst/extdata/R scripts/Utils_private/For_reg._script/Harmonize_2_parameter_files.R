...
#filt <- matrix(c("Parameters csv file for regulation analysis script", "Parameters.csv"), nrow = 1, dimnames = list("proteoCraft", NULL))
# Choose first parameters file
msg1 <- "Select 1st Parameters.csv file"
#fl1 <- normalizePath(choose.files("D:/groups_temp/*.R", msg1, filters = filt), winslash = "/")
fl1 <- rstudioapi::selectFile(msg1,
                              path = "D:/groups_temp/*.R",
                              filter = "R script (*.R)")
wd1 <- dirname(fl1)
setwd(wd1)
# Choose second parameters file
msg2 <- "Select 2nd Parameters.csv file"
#fl2 <- normalizePath(choose.files(paste0(wd1, "/*.R"), msg2, filters = filt), winslash = "/")
fl2 <- rstudioapi::selectFile(msg2,
                              path = paste0(wd1, "/*.R"),
                              filter = "R script (*.R)")
wd2 <- dirname(fl2)
while (wd2 == wd1) {
