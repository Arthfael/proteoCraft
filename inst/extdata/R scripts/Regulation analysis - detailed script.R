...
threshOpt %<o% c(threshMsg, "Absolute log2 FC threshold", "Absolute threshold")
        ...
        if (length(g)) {
          ...
          if (enrichGO) {
            ...
            Kol <- c("Modified sequence", "Evidence IDs", "Sequence", "Proteins", "PEP", "id",
                     "Protein group IDs", "Razor protein group ID", "Leading proteins", "Leading razor proteins",
              ...
              if (length(By)) {
                for (bee in By) { #bee <- By[1]
                  ...
                  if (length(flt)) {
                    ...
                    #temPTM$"First protein" <- sapply(strsplit(temPTM$"Leading proteins", ";"), function(x) { unlist(x)[1] })
