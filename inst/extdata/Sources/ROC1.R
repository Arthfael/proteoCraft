...
if (length(ROCfilt_GOterms_Pos)) {
  ROCfilt_Pos <- unique(unlist(lapply(ROCfilt_GOterms_Pos, \(x) {
...
if (length(c(ROCfilt_Pos, ROCfilt_Neg))) {
  ...
  cat("PSMs filtering using ROC analysis\n")
  ...
  if (length(ROCfilt_Neg)) {
    ...
    if (sum(!tmp2$"Not True Negative") < 10L) {
      cat("Not enough TRUE negatives for ROC analysis (min = 10), skipping!\n")
    } else {
