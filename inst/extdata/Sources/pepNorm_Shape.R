...
for (lGrp in NormGrps$Group) { #lGrp <- NormGrps$Group[1]
  ...
  if (normMeth == "GAM") {
    ...
    unlink(tmpFl)
    tmpDat1b <- setNames(parLapply(parClust, currSamples, \(smpl) {
      A <- A_[[smpl]]
      M <- M_[[smpl]]
...
tmpAB <- lapply(1L:2L, \(i) { #i <- 1L
  ...
  tmpDat_M <- (tmpDat[w,] - avg)/log10(2L)
  tmpDat_A <- (tmpDat[w,] + avg)/2
...
annot_A <- tmpAB[[1L]]$annot_
annot_B <- tmpAB[[2L]]$annot_
...
MAplot <- ggplot(datAB) +
  ggtitle(MAttl) +
  facet_grid(Replicate ~ Sample_group + state) + coord_fixed(0.05) + theme_bw() +
  geom_hex(aes(x = `A (mean log10 Intensity)`, y = `M (mean log2 FC)`, fill = state), size = 1L, alpha = 1, bins = 100L) +
...
whImgs <- 1L:((normMeth == "VSN")+1L)
IMGs <- paste0(shpDr, "/", c(MAttl, SDttl)[whImgs], ".jpeg")
IMGsDims <- as.data.frame(t(parSapply(parClust, IMGs, \(x) { #x <- IMGs[1L]
  a <- jpeg::readJPEG(x)
  setNames(dim(a)[1L:2L], c("height", "width"))
})))
IMGsDims$height <- screenRes$width*c(0.35, 0.3)[whImgs]*IMGsDims$height/max(IMGsDims$height)
IMGsDims$width <- screenRes$width*c(0.35, 0.3)[whImgs]*IMGsDims$width/max(IMGsDims$width)
