...
if (clueGOahead) {
  ...
  Kappa <- c(0.5, 0.8)[m]
  minGenesPerTerm %<o% c(3L, 10L)[m]
  minPercGenesMapped %<o% c(4L, 10L)[m]
  cat(paste0("   | Parameters:",
             ...
             "\n   |> max. GO level = ", maxLvl, 
             "\n   |> Kappa level = ", Kappa, " ]\n"))
  for (nm in names(filters)) { #nm <- names(filters)[1L]
    nm1 <- gsub("___", "_", nm)
    nm2 <- cleanNms(nm)
    ...
    clueTST <- try({
      ...
      for (ii in 1L:2L) { #ii <- 1L #ii <- 2L
        ...
        if (ii == 1L) {
          IDs_lst <- unique(unlist(strsplit(tmpDat[, ID_col], ";"))) # inherited from GO_enrich.R, which should always be run before!
        }
        if (ii == 2L) {
        ...
        if (ii == 2L) {
          IDs_lst2 <- IDs_lst
          stopifnot(sum(!IDs_lst1 %in% IDs_lst2) == 0L)
          IDs_lst <- RJSONIO::toJSON(IDs_lst)
      ...
      response <- httr::GET(rqst, httr::timeout(3600L)) # 1h seems absolutely ginormous!
      if (httr::http_status(response)$category != "Success") { Sys.sleep(2L) }
      ...
      if (!inherits(tst, "try-error")) {
        ...
        for (j in 2L:3L) {
          ...
          g2 <- grep(">", a[g1:l])[1L]
          rg <- g1:(g2+g1-1L)
          gw <- rg[grep("^ *width=\"", a[rg])[1L]]
          gh <- rg[grep("^ *height=\"", a[rg])[1L]]
          ...
          wTxt <- paste0(wTxt[1L], "width=\"", ceiling(w*corr), "px", wTxt[2L])
          hTxt <- paste0(hTxt[1L], "height=\"", ceiling(h*corr), "px", hTxt[2L])
          #vTxt <- paste0(vTxt[1L], "viewBox=\"", paste(c(0, 0, ceiling(w*corr), ceiling(h*corr)), collapse = " "), vTxt[2L])
          a[gw] <- wTxt
          a[gh] <- hTxt
          #a[gv] <- vTxt
          writeLines(a, imgFl)
