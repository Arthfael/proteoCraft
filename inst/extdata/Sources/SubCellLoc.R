...
"
  ...
  if (length(CompGOTerms2)) {
    ...
    CompGOTerms2$Offspring <- lapply(CompGOTerms2$Term, \(x) { GOCCOFFSPRING[[x]] })
    ...
    if (length(SubCellMark2)) {
      ...
      f0 <- \(grp) { #grp <- SubCellFracAggr$values[1L]
        ttl_s <- c()
        grp1 <- cleanNms(grp)
        ...
        if (length(w)) {
          ...
          sd <- sd(unlist(temp1))
          noise <- rnorm(length(unlist(temp1)), 0, sd*10L^-6)
          temp1 <- temp1 + noise
          #
          temp1 <- 10L^temp1
          tst <- rowSums(temp1)
          ...
          if (length(wAG)) {
            ...
            temp1 <- sweep(temp1[wAG,], 1L, tst[wAG], "/")
  ...
  if (max(tst) == 1L) {
    ...
    if ((length(wh0) == 1L)&&(length(wh1))) {
      ...
      comb <- gtools::combinations(max(EM0$Replicate), 2L, unique(EM0$Replicate))
      ...
      testI <- data.frame(Intensity = (binz[2L:(nbinz+1L)]+binz[seq_len(nbinz)])/2L)
      ...
      #
      volcPlot_args2 <- volcPlot_args2
      volcPlot_args2$Prot <- temp
      volcPlot_args2$FDR.thresh <- SSD.FDR.thresh
      volcPlot_args2$FDR.root <- "Signif. SSDs-FDR="
      volcPlot_args2$title <- "SSDs volcano plot "
      volcPlot_args2$subfolder <- subDr
      volcPlot_args2$Symmetrical <- FALSE
      volcPlot_args2$X.root <- paste0("Mean ", SSD.Root)
        ...
        for (gi in g) { #gi <- g[1L]
          ...
          if (length(wUp)) {
            ...
            for (wup in wUp) { #wup <- wUp[1L]
              # Protein heatmap
              Seq <- db$Sequence[match(unlist(strsplit(PG$`Leading protein IDs`[wup], ";"))[1L], db$`Protein ID`)]
              ...
              Prfl$value <- 10L^as.numeric(PG[wup, paste0(PGRoot, Prfl$Sample)])
              ...
              Match <- vapply(pepPrfl$Sequence, \(x) { nchar(unlist(strsplit(Seq, x))[1L]) }, 1L)
              ...
              temp3 <- aggregate(temp$y, list(temp[[SubCellFracAggr$aggregate]]), \(x) { list(Min = min(x),
                                                                                                     Max = max(x)+1) })
              colnames(temp3) <- c("Samples group", "x")
              temp3[, c("Min", "Max")] <- apply(temp3$x, 2L, as.numeric)
              temp3$Mean <- apply(temp3[, c("Min", "Max")], 1L, mean)
              ...
              temp4 <- aggregate(temp$y, list(temp[[SubCellFracAggr2$aggregate]]), \(x) { list(Min = min(x),
                                                                                                      Max = max(x)+1) })
              colnames(temp4) <- c("Samples group 2", "x")
              temp4[, c("Min", "Max")] <- apply(temp4$x, 2L, as.numeric)
              temp4$Mean <- apply(temp4[, c("Min", "Max")], 1L, mean)
              temp4$"Samples group 2" <- cleanNms(temp4$"Samples group 2")
              temp5 <- aggregate(temp$y, list(temp$Sample), \(x) { list(Min = min(x), Max = max(x)+1) })
              colnames(temp5) <- c("Sample", "x")
              temp5[, c("Min", "Max")] <- apply(temp5$x, 2L, as.numeric)
              temp5$Mean <- apply(temp5[, c("Min", "Max")], 1L, mean)
