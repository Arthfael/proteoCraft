...
if (goAhead) {
  ...
  exports <- list("prot.names", "IDs.list", "TEST0", "tmpDB", "smplLvl", "TMP0", "nCharLim", "wd", "rfRoot", "smplCol", "scrptType",
                  "WorkFlow", "runRat", "Coverage", "listMelt", "poplot", "annot_to_tabl", "cleanNms", "Exp", "topattern")
  if (scrptType == "withReps") {
    exports <- append(exports, list("cleanNms", "Exp"))
  }
  if (runRat) {
    exports <- append(exports, list("comb", "myComb", "pepR"))
  }
  clusterExport(parClust, exports, envir = environment())
  tmpPlots <- setNames(parLapply(parClust, whMtch, \(iii) { #iii <- whMtch[1L]
    RES <- list()
    p <- prot.names[iii]
    IDs <- IDs.list[[p]]
    w <- which(TEST0$value %in% IDs)
    ...
    for (id in IDs) { #id <- IDs[1L]
      ...
      tempev <- setNames(lapply(smplLvl, \(exp) { #exp <- smplLvl[1L]
