...
if ((exists("db"))&&("Protein of interest" %in% colnames(db))) {
  ...
  prot.list <- union(prot.list, db$`Protein ID`[which(db$`Protein of interest`)])
}
if ((exists("Param"))&&("Prot.list" %in% colnames(Param))) {
  prot.list <- union(prot.list, unlist(strsplit(Param$Prot.list, ";")))
}
if ((exists("Param"))&&("Prot.list_pep" %in% colnames(Param))) {
  prot.list_pep <- unlist(strsplit(Param$Prot.list_pep, ";"))
  prot.list <- union(prot.list, prot.list_pep)
}
if ((exists("Param"))&&("Norma.Prot.Ratio.to.proteins" %in% colnames(Param))) {
  prot.list <- union(prot.list, unlist(strsplit(Param$Norma.Prot.Ratio.to.proteins, ";")))
}
if (exists("TargetProteins")) { prot.list <- union(prot.list, TargetProteins) }
prot.list %<o% unique(prot.list)
# Update backup fasta
if (length(prot.list)) {
  m <- match(prot.list, db$`Protein ID`)
  m <- unique(m[which(!is.na(m))])
