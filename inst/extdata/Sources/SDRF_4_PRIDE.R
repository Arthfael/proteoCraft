...
if (tstOrg) {
  ...
  mainOrg <- tst$Group.1[1L]
} else {
  dbOrgKol <- c()
  mainOrg <- "[UNKNOWN_ORGANISM]"
}
if (!nchar(mainOrg)) {
  mainOrg <- dlg_input("Enter the species of the samples")$res
}
dbOrgKol %<o% dbOrgKol
tstOrg %<o% tstOrg
mainOrg %<o% mainOrg
...
if (isFnd["MS_models"]) {
  ...
  Vendors2Instr <- aggregate(availInstrDF$Instrument, list(availInstrDF$Vendor), \(x) { sort(unique(x)) })
...
serverA <- \(input, output, session) {
  VENDOR <- reactiveVal(myVendor)
  MSINSTR <- reactiveVal(myMSInstr)
  if (!tstOrg) {
    ORGA <- reactiveVal(mainOrg)
  }
  # MS instrument update function
  updtMSInstr <- \(reactive = TRUE) {
    if (reactive) {
      myVend <- VENDOR()
      myInstr <- MSINSTR()
    } else {
...
if (scrptType == "withReps") {
  ...
  if (LabelType == "Isobaric") {
    tmp <- listMelt(Exp.map$MQ.Exp, 1L:nrow(Exp.map), c("MQ.Exp", "ExpRow"))
...
if (scrptType == "noReps") {
  ...
  myKol <- setdiff(myKol, c("Fractions", "Reference", "MQ.Exp", "Sample.name", "Use"))
  myKol <- grep("__$", myKol, value = TRUE, invert = TRUE)
  tmp <- listMelt(SamplesMap$Experiment, 1L:nrow(SamplesMap))
...
if (!"assay name" %in% colnames(SDRF)) { SDRF$"assay name" <- paste0("Run ",  1L:nrow(SDRF)) }
if (!"technology type" %in% colnames(SDRF)) { SDRF$"technology type" <- "proteomic profiling by mass spectrometry" }
...
if (!"comment[technical replicate]" %in% colnames(SDRF)) {
  SDRF$"comment[technical replicate]" <- 1L # Please note so far none of our workflows support technical replicates!!!
}
...
if (!"comment[cleavage agent details]" %in% colnames(SDRF)) {
  # Only trypsin is supported so far!
...
myOntDF$Candidate_col_root <- list(c("Cell Type", "Cell type"),
                                   ...
                                   NA,
                                   NA,
                                   c(NA, "Isobaric label details")[(LabelType == "Isobaric") + 1L],
                                   "PTM-enriched")
...
for (i in wNotNA) { #i <- wNotNA[1L]
  nm <- myOntDF$Name[i]
  myOntVal <- get(paste0(myOntDF$myOntology[i], "2"))
  if (length(myOntVal) == 1L) {
...
#
if (idsL) {
  wTest1 <- vapply(colnames(SDRF2), \(k) { #k <- colnames(SDRF2)[1L]
    if (nchar(k)) {
  ...
  wTest2 <- sum(wTest1) + 15L + ncol(SDRF2)*5L
        ...
        observeEvent(input[[id2]],
                     {
                       ...
                       for (k in (i+1L):nr) {
                         ...
                         updateSelectInput(getDefaultReactiveDomain(), idK, NULL, myOntVal, x)
                       }
                     })
...
writeExprs <- expression({
  tst <- try({
    write.table(SDRF, sdrfPth, sep = "\t", quote = FALSE, row.names = FALSE, fileEncoding = "UTF-8")
  }, silent = TRUE)
})
eval(writeExprs)
