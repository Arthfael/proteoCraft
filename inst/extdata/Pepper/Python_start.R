...
dfltLocFl <- paste0(homePath, "/Default_locations.xlsx")
dfltLoc <- openxlsx2::read_xlsx(dfltLocFl)
if ("Python" %in% locDirs$Folder) { 
  pyPath <- gsub("/", "\\\\", locDirs$Path[match("Python", locDirs$Folder)])
} else {
