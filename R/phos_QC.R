...
phos_QC <- function(PhPep,
  ...
  M <- annot_to_tabl(PhPep[[ModSeq_col]])
  P <- annot_to_tabl(PhPep[[P_col]], Nterm = FALSE, Cterm = FALSE, numeric_data = TRUE)
  ScD <- annot_to_tabl(PhPep[[ScD_col]], Nterm = FALSE, Cterm = FALSE, numeric_data = TRUE)
