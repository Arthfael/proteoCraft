...
PD_to_MQ <- function(PD,
  ...
  if (Isobaric) {
    ...
    LabelKlass <- LabelKlass[which(vapply(LabelKlass, function(x) { grepl(topattern(x), Label) }, TRUE))]
  ...
  PD[w, c("temp_mod", "temp_pos")] <- Isapply(temp[w], function(x) { #x <- temp[[1]]
    x <- unlist(x)
  ...
  EV$"Modified sequence"[w] <- apply(temp, 1, function(x) {
    ...
    if (length(md) > 0) {
      wh <- Isapply(1:length(wh), function(y) { c(substr(wh[y], 1, 1), substr(wh[y], 2, nchar(wh[y]))) })
  ...
  if (("Identifying Node" %in% colnames(PD))&&(length(unique(PD$"Identifying Node")) > 1)) {
    ...
    EV$test <- do.call(paste, c(apply(EV[, c("Modified sequence", "Charge", "Retention time", "Raw file", "MS/MS scan number")], sep = "_-_"))
