...
Volcano.plot <- function(Prot,
  ...
  if (length(wOK)) {
    Wych <- setNames(lapply(wOK, function(x) {
      wych <- (is.all.good(as.numeric(Prot[[xKols[x]]]), 2))&(is.all.good(as.numeric(Prot[[yKols[x]]]), 2))
  ...
  if (length(weech)) {
    ...
    Prot$Labels[weech] <- parallel::parSapply(cl, strsplit(Prot$Labels[weech], "  ?"), .labelEdit_worker)
  }
  weech <- which(Prot$Labels == "")
  ...
  for (i in A) { #i <- A[1]
    ...
    if (!is.numeric(Alpha)) {
      if (!Alpha %in% colnames(Prot)) {
        ...
        if (!Alpha.identity) {
          ...
          temp$Alpha[which((temp$Alpha > 0)&(is.infinite(temp$Alpha)))] <- max(is.all.good(temp$Alpha))
          temp$Alpha[which(!is.all.good(temp$Alpha, 2))] <- min(is.all.good(temp$Alpha))
    ...
    if (!is.numeric(Size)) {
      if (!Size %in% colnames(Prot)) {
        ...
        temp$Size[which((temp$Size > 0)&(is.infinite(temp$Size)))] <- max(is.all.good(temp$Size))
        temp$Size[which(!is.all.good(temp$Size, 2))] <- min(is.all.good(temp$Size))
    ...
    if (!is.null(Ref.Ratio.values)) {
      ...
      if (Ref.Ratio.method == "SD") {
        offset <- c(median(is.all.good(temp$X)), 0)[X.normalized + 1]
        sdx <- sd(x$value)
