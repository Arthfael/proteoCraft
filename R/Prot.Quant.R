...
Prot.Quant <- function(Prot,
                       ...
                       cl) {
  TESTING <- FALSE
  #DefArg(Prot.Quant);TESTING <- TRUE; cl <- parClust
  ...
  if (!misFun(param)) {
    ...
    ref.groups <- parse.Param.aggreg(param$Ratios.Ref.Groups,
                                                  aggregates,
                                                  aggregate.map,
                                                  aggregate.list)
    ...
    ratio.groups <- parse.Param.aggreg(param$Ratios.Groups,
                                                    aggregates,
                                                    aggregate.map,
                                                    aggregate.list)
    sample.groups <- parse.Param.aggreg(param$Volcano.plots.Aggregate.Level,
                                                     aggregates,
                                                     aggregate.map,
                                                     aggregate.list)
  }
  ...
  if (Mode == "PREFERUNIQUE") {
    ...
    f0 <- function(x) { mean(is.all.good(x)) }
    environment(f0) <- .GlobalEnv
  ...
  if (Priority == "rat") {
    ...
    ratios.fun <- function(x) { #x <- temp.ids[1]
      ...
      if (length(mtch)) {
        ...
        if (length(Pep.Ratios.Nms) >= Min.N) {
          tst1 <- apply(temp[, Pep.Ratios.Nms, drop = FALSE], 1, function(y) { length(is.all.good(y)) }) > 0
          if (sum(tst1)) {
            ...
            if (sum(tst1) > Max.N) { # Remove excess peptides, for cases where we have much more than we need and including all would slow down the calculations (Titin!!!)
              temp$Rank <- nrow(temp) + 1 - rank(apply(temp[, Pep.Intens.Nms, drop = FALSE], 1, function(y) { sum(is.all.good(y)) }))
    ...
    colnames(res) <- sapply(Pep.Ratios.Nms, function(x) {
      sapply(cola, function(y) { gsub(topattern(gsub(" - $", "", Pep.Ratios.root)), paste0(gsub(" - $", "", Pep.Ratios.root), y), x) })
    })
    # Reorder:
    res <- res[, sapply(cola, function(x) {
      gsub(topattern(gsub(" - $", "", Pep.Ratios.root)), paste0(gsub(" - $", "", Pep.Ratios.root), x), Pep.Ratios.Nms)
    })]
  ...
  #grsep(prot.list[2], x = Prot$`Leading protein IDs`)
  #View(ord)
  f0 <- function(ids) {
    LFQ.lm(ids,
                        InputTabl = tmpPep,
  ...
  colnames(res2) <- gsub(topattern(Pep.Intens.root), Expr.root.full, Pep.Intens.Nms)
  ...
  if (!Skip.ratios) {
    ...
    rat.2.ref <- lapply(ref.groups$values, function(i) { #i <- ref.groups$values[1]
      ...
      if (length(b)) {
        ...
        if (length(w)) {
          b1 <- as.numeric(apply(res2[, kol[w], drop = FALSE], 1, log_ratio_av)) # (Expression data is already log-transformed for Prot.Quant)
    ...
    for (i in ref.groups$values) { #i <- ref.groups$values[1]
      ...
      if (!length(a)) {
        ...
        if (PrintSummary) {
          for (a1 in A) { #a1 <- A[1]
            tmp <- is.all.good(res2[[paste0(Expr.root.full, a1)]])
            ...
            if (paste0(Pep.Ratios.root, a1) %in% colnames(res2)) {
              tmp <- is.all.good(res2[[paste0(Pep.Ratios.root, a1)]])
  ...
  if (!Prot.LFQ.to.log) { # De-log LFQ values
    g <- grep(topattern(Expr.root.full), colnames(res2), value = TRUE)
    res2[, g] <- log.Pep.Intens^(res2[, g])
    colnames(res2) <- gsub(topattern(Expr.root.full),
                           "Expr.",
                           colnames(res2))
  }
  if (!Skip.ratios) {
