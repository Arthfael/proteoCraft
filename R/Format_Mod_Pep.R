...
Format_Mod_Pep <- function(x) {
  ...
  mod$Short <- substr(mod$Full, 2L, 2L)
  mod$Short[which(! mod$Short %in% y)] <- NA
  if (length(which(!y %in% unlist(mod)))) { warning("Something went awry!") }
  mod$Specificity <- NA
  ...
  for (w in W) {
    ...
    x3 <- Isapply(x3, \(z) {
      ...
      res <- sapply(z[2L:l], \(s) {
        n <- nchar(s)
        s1 <- substr(s, 2L, 2L)
        s2 <- if (n > 2L) { substr(s, 3L, n) } else { "" }
        s <- paste0(s1, mod$Full[w], s2)
        return(list(c(s, s1)))
      })
      res1 <- paste(c(z[1L], sapply(res, \(r) { r[[1L]] })), collapse = "")
      res2 <- list(unique(sapply(res, \(r) { r[[2L]] })))
      return(c(res1, res2))
    })
    x3a <- unlist(x3[, 1L])
    ...
    mod$Specificity[w] <- paste(sort(unique(unlist(x3[, 2L]))), collapse = ";")
  }
  x <- xtemp[match(x, x1)]
