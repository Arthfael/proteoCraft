...
AA_bias <- function(Ev,
  ...
  AA <- data.table::data.table(E = E,
                               dumdum = as.integer(rep(1L, length(E))))
  ...
  AA_db <- data.table::data.table(D = D,
                                  dumdum = as.integer(rep(1L, length(D))))
  ...
  AA$Ratio <- signif(AA$"% - dataset"/AA$"% - database", 3L)
  AA$"% - dataset" <- signif(AA$"% - dataset", 3L)
  AA$"% - database" <- signif(AA$"% - database", 3L)
  return(AA)
}
