...
PG_assemble <- function(Pep,
  ...
  f0 <- function(x) { x[which(x %in% tmp2)] }
  environment(f0) <- .GlobalEnv
  seq$.Proteins <- parallel::parLapply(cl, tmp1, f0)
  wY <- which(sapply(seq$.Proteins, length) > 0)
  wN <- which(sapply(seq$.Proteins, length) == 0)
  if (length(wN) > 0) { warning("Not all of the protein IDs in the peptides file are in the data base!") }
  seq <- seq[wY,]
  seq$Proteins <- sapply(seq$Proteins, paste, collapse = ";")
  ...
  if (CustPG) {
    ...
    if (length(g)) {
      ...
      prot2$"Peptides count" <- sapply(prot2$.pep.ids, length)
      ...
      Custom_PGs$"Peptide IDs" <- sapply(Custom_PGs$.pep.ids, paste, collapse = ";")  
      Custom_PGs$.lead.protein.ids <- lapply(Custom_PGs$.Leading.protein.IDs, function(x) {
        prot2$Prot.id[match(x, prot2$Protein)]
      })
      Custom_PGs$lead.protein.ids <- sapply(Custom_PGs$.lead.protein.ids, paste, collapse = ";")
      Custom_PGs$"Peptides count" <- sapply(Custom_PGs$.pep.ids, length)
      Custom_PGs$temp.pg.id <- as.character(1:nrow(Custom_PGs))
      pgoffset <- nrow(Custom_PGs)
      seq2$.Proteins <- strsplit(seq2$Proteins, ";")
      seq2$.Prot.ids <- sapply(seq2$.Proteins, function(x) { prot2$Prot.id[match(unlist(x), prot2$Protein)] })
  ...
  # Sort by N of peptides
  prot$"Peptides count" <- sapply(prot$.pep.ids, length)
  ...
  tmp1 <- seq$.Proteins
  tmp2 <- prot[, c("Prot.id", "Protein")]
  parallel::clusterExport(cl, list("tmp1", "tmp2"), envir = environment())
  f0 <- function(x) { list(tmp2$Prot.id[match(unlist(x), tmp2$Protein)]) }
  environment(f0) <- .GlobalEnv
  seq$.Prot.ids <- parallel::parSapply(cl, tmp1, f0)
  ...
  f0 <- function(x) { sapply(unlist(x), function(y) { tmp1$.pep.ids[match(y, tmp1$Prot.id)] }) }
  environment(f0) <- .GlobalEnv
  ...
  tmp1 <- prot[, c(".pep.ids", "pep_to_P", "pep_to_P_to_pep")]
  parallel::clusterExport(cl, "tmp1", envir = environment())
  f0 <- function(x) {
    #x <- tmp1[1,]
    x1 <- unlist(x[[1]]) #This protein's peptides
    x3 <- x[[3]] # All peptides from other proteins this protein shares some peptides with
    #x3 <- x3[[1]] #(for testing)
    x3n <- unlist(x[[2]]) # 
    x3l <- sapply(x3, length)
    test <- sapply(x3, function(y) { sum(y %in% x1) }) == x3l # Can the 2nd protein be subsumed in the 1st?
    res <- x3n[which(test)]
    return(res)
  }
  environment(f0) <- .GlobalEnv
  protTemp <- parallel::parApply(cl, tmp1, 1, f0)
  ...
  w <- which(sapply(prot$Contains, length) > 0)
  temp <- proteoCraft::listMelt(prot$Contains[w], prot$Prot.id[w])
  ...
  tmp1 <- contained$.byWhom
  tmp2 <- prot[, c(".pep.ids", "Prot.id")]
  exports <- list("tmp1", "tmp2")
  parallel::clusterExport(cl, exports, envir = environment())
  f0 <- function(x) { tmp2$.pep.ids[match(unlist(x), tmp2$Prot.id)] }
  environment(f0) <- .GlobalEnv
  contained$Container.pep.ids <- parallel::parSapply(cl, tmp1, f0)
  ...
  pg$"Peptides count" <- sapply(pg$.pep.ids, length)
  ...
  pg$"Protein IDs" <- sapply(pg$.Protein.IDs, function(x) { paste(unlist(x), collapse = ";") })
  if (CustPG) {
    Custom_PGs$"Also contains" <- sapply(Custom_PGs$.lead.protein.ids, function(x) {
    ...
    Custom_PGs$"Protein IDs" <- sapply(Custom_PGs$.Protein.IDs, function(x) { paste(unlist(x), collapse = ";") })
  }
  ...
  if (CustPG) {
    ...
    Custom_PGs$.PEPs <- sapply(Custom_PGs$.Protein.IDs, function(x) { prot$PEP[match(x, prot$Protein)] })
    Custom_PGs$PEPs <- sapply(Custom_PGs$.PEPs, function(x) { paste(x, collapse = ";") })
    Custom_PGs$PEP <- sapply(Custom_PGs$.PEPs, min)
  }
  ...
  if (length(W)) {
    ...
    pt <- pt[order(sapply(pt$.temp.pg.ids, length), decreasing = TRUE),]
    ...
    while (sum(!pt$Done)) {
      ...
      tst0 <- sapply(pg$.pep.ids, function(x) { length(which(unlist(x) %in% p)) }) > 0
      ...
      # Extend overlappers:
      tst2 <- sapply(pg$.pep.ids, function(x) { length(which(unlist(x) %in% P)) }) > 0 # Proteins with at least one "P" peptide
      ...
      while (L2 > L) {
        #kount <- kount + 1
        P <- P2; L <- L2
        tst2 <- sapply(pg$.pep.ids, function(x) { length(which(unlist(x) %in% P)) }) > 0
        overlappers2 <- which(tst2)
        P2 <- unique(unlist(pg$.pep.ids[overlappers2])); L2 <- length(P2)
        #cat(paste0("   ", kount, "\n"))
      ...
      tst1a <- sapply(pg$.pep.ids[removables], length) # Their N of peptides
      tst1b <- pg$PEP[removables] # Their PEPs
      tst1b[which(is.na(tst1b))] <- 1
      go <- length(removables) > 0
      while (go) {
        tst2 <- sapply(removables, function(x) {
          length(unique(unlist(pg$.pep.ids[keepers[which(keepers != x)]])))
        }) == L # Can any of these be dropped without affecting the number of peptides?
        witch <- which(tst2)
        if (length(witch)) {
  ...
  if (!sum(!p %in% p1)) { # (Sanity check)
    ...
    pg[[pepcolnm]] <- sapply(pg$.pep.ids, paste, collapse = ";")
    ...
    environment(f0) <- .GlobalEnv
    seq$.PG.ids <- parallel::parSapply(cl, tmp1, f0)
    exports <- list("seq")
    parallel::clusterExport(cl, exports, envir = environment())
    f0 <- function(x) { paste(x, collapse = ";") }
    environment(f0) <- .GlobalEnv
    seq$"Protein group IDs" <- parallel::parSapply(cl, seq$.PG.ids, f0)
    pg$temp.pg.id <- NULL
    ...
    pg$"Unique peptide IDs" <- sapply(pg$.unique.pep.ids, paste, collapse = ";")
    pg$"Unique peptides" <- sapply(pg$.unique.pep.ids, length)
    # Apply Occam's razor:
    ...
    tmp$Razor_PG_IDs <- sapply(tmp$.Razor_PG_IDs, paste, collapse = ";")
    ...
    tmp2 <- prot[, c("PEP", "Protein")]
    tmp3 <- seq[, c2]
    exports <- list("seq", "tmp1", "tmp2", "tmp3", "c2")
    parallel::clusterExport(cl, exports, envir = environment())
    f0 <- function(x) {
    ...
    # Update columns
    seq$"Razor protein group ID" <- sapply(seq$.Razor.protein.group.ID, paste, collapse = ";")
    seq$"Protein group ID" <- sapply(seq$.Protein.group.IDs, paste, collapse = ";")
    #
    seq$"Leading razor proteins" <- sapply(strsplit(seq$"Leading razor proteins", ";"), function(x) { unlist(x)[1] })
    ...
    pg$"Razor + unique peptides" <- sapply(pg$.razor.pep.ids, length)
    ...
    # 
    pg$"Peptide is razor" <- sapply(pg$.Peptide.is.razor, paste, collapse = ";")
    ...
    exports <- list("tmp1", "tmp2", "c2")
    parallel::clusterExport(cl, exports, envir = environment())
    f0 <- function(x) {
      x1 <- unlist(x[[1]]); x2 <- unlist(x[[2]]); x3 <- unlist(x[[3]]); x4 <- unlist(x[[4]])
      x4 <- tmp2$.pep.ids[match(x4, tmp2$Protein)]
      x1 <- paste(sapply(x4, function(y) { length(which(y %in% x1)) }), collapse = ";")
      x2 <- paste(sapply(x4, function(y) { length(which(y %in% x2)) }), collapse = ";")
      x3 <- paste(sapply(x4, function(y) { length(which(y %in% x3)) }), collapse = ";")
      return(c(x1, x2, x3))
    }
    environment(f0) <- .GlobalEnv
    ...
    if (Npep > 1) {
      pg[[paste0("Quality filter: min ", Npep, " razor&unique pep.")]] <- c("", "Keep")[
        sapply(strsplit(pg$`Peptide counts (razor+unique)`, ";"), function(x) {
          as.integer(x[[1]]) >= Npep
        }) + 1]
    }
    #
    ...
    exports <- list("tmp1", "tmp2", "c2", "tmp3", "Coverage")
    parallel::clusterExport(cl, exports, envir = environment())
    f0 <- function(x) {
      #x <- tmp2[1, c2]
    ...
    temp2 <- strsplit(temp2b, ";")
    temp1 <- temp[, c("Protein ID", c2)]
    exports <- list("temp1", "temp2", "c1", "c2", "ca", "cb")
    parallel::clusterExport(cl, exports, envir = environment())
    f0 <- function(x) {
      x <- unlist(x)
      w <- match(x, temp1$"Protein ID")
      res <- temp1[w, c2]
      if (length(w) > 1) {
    ...
    environment(f0) <- .GlobalEnv
    tst2 <- parallel::parLapply(cl, temp2, f0)
    tst2 <- as.data.frame(t(as.data.frame(tst2)))
    pg[, c1] <- tst2
    #sapply(1:4, function(x) { sum(tst1[[x]] != tst2[[x]])})
    ...
    tmp2 <- sapply(strsplit(pg$"Leading protein IDs", ";"), function(x) { x[1] })
    exports <- list("tmp", "tmp2")
    parallel::clusterExport(cl, exports, envir = environment())
    ...
    test <- nchar(gsub(paste(c(AA, "_"), collapse = "|"), "", seq$"Modified sequence"))
    tmp1 <- pg$.pep.ids
    tmp2 <- seq$id
    exports <- list("tmp1", "test", "tmp2")
    parallel::clusterExport(cl, exports, envir = environment())
    f0 <- function(x) {
      x <- unlist(x)
      x <- test[match(x, tmp2)]
    ...
    if ("Common Name" %in% colnames(DB)) {
      ...
      pg$"Common Name (short)" <- sapply(strsplit(pg$"Common Names", ";"), function(x) {
        x <- unlist(x)[1]
        if (!length(x)) { x <- "" }
        return(x)
      })
    ...
    res <- list(Protein.groups = pg, Peptides = Pep, Database = DB)
    if (!misFun(Ev)) { res$Evidences <- Ev }
