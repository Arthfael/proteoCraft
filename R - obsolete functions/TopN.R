...
TopN <- function(N = 3,
  ...
  test <- rep(0, nrow(Pep))
  if (sum(Mods != FALSE) != 0) {
    a <- paste(c(paste(c(AA, "_"), collapse = "|"), sapply(Mods, function(x) {
      paste0("\\(", x, "\\)")
    })), collapse = "|")
  ...
  res <- sapply(temp.ids, function(x) { #x <- temp.ids[1]
    ...
    if (length(x)) {
      if (length(Pep.Intens.Nms) > 1) {
        ...
        temp <- is.all.good(pep1[x, Pep.Intens.Nms])
        if (length(temp) >= N) {
  ...
  if (N > 1) {
      ...
      for (o in Pep.Intens.Nms) {
        ...
        for (n in 1:(N-1)) {
          ...
          y2 <- apply(y1, 1, function(x) {
            x <- is.all.good(as.numeric(x))
            if (length(x) == 2) { return(TRUE) } else { return(TRUE) }
          })
          options(warn = 0)
          y1 <- y1[which(y2),]
    ...
    if (corr == "global") {
      ...
      for (n in 1:(N-1)) {
        ...
        y1$N <- sapply(y, function(x) { unlist(x)[N] })
        options(warn = -1)
        y2 <- apply(y1, 1, function(x) { length(is.all.good(as.numeric(x))) }) == 2
        options(warn = 0)
        y1 <- y1[which(y2),]
  ...
  if (Out.Norm) {
    for (i in colnames(res2)) {
      a <- median(is.all.good(unlist(res2[[i]])))
      res2[[i]] <- res2[[i]] - a
    }
  }
  if (!Prot.TopN.to.log) {
